...
describe("transform", () => {
  interface Transformer {
    (code: string, fileName: string): Promise<{
      code: string;
      map: {
        file: string;
        mappings: string;
  ...
  beforeEach(() => {
    ...
    transformStylePreprocessing = PluginVue({ customBlocks: ["*"], preprocessStyles: true })
      .transform as typeof transform;
    resolveId = PluginVue({ customBlocks: ["*"] }).resolveId as typeof resolveId;
    resolveIdNonMatching = PluginVue({ include: ["none"], exclude: ["example.vue"] })
      .resolveId as typeof resolveId;
  ...
  it("should transform <template> block with `ssrRender`", async () => {
    ...
    expect(code).toEqual(
      expect.stringContaining(
        `import { ssrRender } from "example.vue?vue&type=template&id=4b16ad9e"`,
      ),
    );
  ...
  it("should load custom block with src", async () => {
    const transformResult = await transform(
      `<customTag src="someSource.ext">something</customTag>`,
      `example.vue`,
    );
    expect(transformResult).toEqual(
      expect.objectContaining({
  ...
  it("should ignore including custom block with non-matching `customBlocks`", async () => {
    const transformResult = await transformNonMatchingBlocks(
  ...
  it("should include explicitly included custom block", async () => {
    const transformResult = await transformNonMatchingBlocks(
      `<okCustomTag>something</okCustomTag>`,
      `example.vue`,
    );
    expect(transformResult).toEqual(
      expect.objectContaining({
  ...
  it("should return `null` with transform of vue query string missing a matching `type`.", async () => {
    ...
    const result = await transform(
      ...
      `example.vue?vue&id=example.vue&index=0`,
    );
  ...
  it("should return `null` with transform of vue query string with excluded file name.", async () => {
    ...
    const result = await transformNonMatching(
      `<style scoped>.foo {}</style>`,
      `example.vue?vue&id=example.vue&index=0`,
    );
  ...
  it("should return code/map object with transform of vue query string including `id`, scoped style, and a matching template `type`.", async () => {
    await transformSSR(
      `<style scoped>a {color: red;}</style><template><div /></template>`,
      `example.vue`,
    );

    const result = await transformSSR(
      `<style scoped>a {color: red;}</style><template><div /></template>`,
      `example.vue?vue&id=example.vue&index=0&type=template`,
    );
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        map: {
          ...
          sources: [expect.stringMatching("rollup-plugin-vue3/example.vue")],
          sourcesContent: [null],
          version: 3,
        },
      }),
  ...
  it("should return code/map object with transform of vue query string and a matching template `type`.", async () => {
    ...
    const result = await transform(
      ...
      `example.vue?vue&index=0&type=template`,
    );
  ...
  it("should return code/map object with transform of vue query string including `id` and a matching style `type`.", async () => {
    ...
    const result = await transform(
      `<style>.foo {}</style>`,
      `example.vue?vue&id=example.vue&index=0&type=style`,
    );
  ...
  it("should return code/map object with transform of vue query string and a matching style `type` and `module`.", async () => {
    ...
    const result = await transform(
      `<style module>.foo {}</style>`,
      `example.vue?vue&id=example.vue&index=0&type=style&module=1`,
    );
  ...
  it("should return code/map object with transform of vue query string and a matching style `type` and `vars`.", async () => {
    ...
    const result = await transform(
      `<style vars="abc">.foo {}</style>`,
      `example.vue?vue&id=example.vue&index=0&type=style&vars=abc`,
    );
  ...
  color: red;
    ...
    color: red`,
      ...
      await transformStylePreprocessing(
        `<style lang="${preprocessLang}">${input}</style>`,
        `example.vue`,
      );
      // eslint-disable-next-line no-await-in-loop
      const result = await transformStylePreprocessing(
        `<style lang="${preprocessLang}">>${input}</style>`,
        `example.vue?vue&id=example.vue&index=0&type=style`,
      );
      expect(result).toEqual(
  ...
  it("should process tips with transform of vue query string and a matching `type`.", async () => {
    ...
    const result = await transformPreprocess.call(
      ...
      `<template lang="md"># Hello</template>`,
      `example.vue?vue&id=example.vue&index=0&type=template`,
    );

    expect(warningObject).toEqual(
      expect.objectContaining({
        id: "example.vue",
        message:
          "Component example.vue uses lang md for template. Please install the language preprocessor.",
      }),
    );
  ...
  it("should return with transform of template with `src`.", async () => {
    const result = await transform(
      `<template src="someSource.html"><div /></template>`,
      `example.vue`,
    );
    expect(result).toEqual(
      expect.objectContaining({
  ...
  it("should return with transform of script and template with `src`.", async () => {
    await transform(
      `<template src="someSource.html"><div /></template><script>export default {};</script>`,
      `example.vue`,
    );
    const result = await transform(
      `<template src="someSource.html"><div /></template>`,
      `example.vue?vue&src&type=template`,
    );
    expect(result).toEqual(
      expect.objectContaining({
  ...
  it("should return with transform of script with `src`.", async () => {
    const result = await transform(
      `<script src="test.js">export default {}</script>`,
      `example.vue`,
    );
    expect(result).toEqual(
      expect.objectContaining({
  ...
  it("should return with transform of style with `src`.", async () => {
    const result = await transform(
      `<style src="test.css">export default {}</style>`,
      `example.vue`,
    );
    expect(result).toEqual(
      expect.objectContaining({
...
`,
  ...
  it("should return null with vue query load with no matching type.", async () => {
    ...
    await resolveId.call(
      ...
      `./__tests__/fixtures/example.vue?vue&src`,
      "example.vue",
    );
